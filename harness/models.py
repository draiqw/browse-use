...
PROVIDERS: dict[str, Provider] = {p.name: p for p in [
    Provider("openai", "ChatOpenAI", ("OPENAI_API_KEY",),
             aliases=("gpt-", "o1", "o3", "o4")),
    Provider("anthropic", "ChatAnthropic", ("ANTHROPIC_API_KEY",),
             aliases=("claude-",)),
    Provider("google", "ChatGoogle", ("GOOGLE_API_KEY", "GEMINI_API_KEY"),
             aliases=("gemini-",)),
    Provider("groq", "ChatGroq", ("GROQ_API_KEY",)),
    Provider("deepseek", "ChatDeepSeek", ("DEEPSEEK_API_KEY",),
             aliases=("deepseek-",)),
    Provider("openrouter", "ChatOpenRouter", ("OPENROUTER_API_KEY",),
             note="имя модели вида qwen/qwen3-235b"),
    ...
    Provider("browseruse", "ChatBrowserUse", ("BROWSER_USE_API_KEY",),
             note="облачная модель самих browser-use"),
    Provider("ollama", "ChatOllama", (), note="локально, ключ не нужен; host в OLLAMA_HOST"),
    Provider("compat", "ChatOpenAI", (),
             note="любой OpenAI-совместимый эндпоинт: vLLM, LM Studio, together — задай base_url"),
]}


def _fields(cls) -> set[str]:
    """Классы browser-use — dataclass'ы, а не pydantic-модели. Но апстрим волен это поменять,
...
def _load(cls_name: str):
    ...
    if cls is None:
        raise RuntimeError(
            ...
            f"Похоже, апстрим переименовал провайдера — проверь `python -m harness doctor`."
        )
    return cls
...
def make_model(
    ...
    **extra,
    ...
    """Дефолт browser-use — 4096 токенов на ответ. Длинная таблица туда не влезает,
    структурированный вывод обрезается на середине, прогон уходит в мусор. Поднимаем явно."""
...
def available() -> list[tuple[str, bool, str]]:
    ...
    for name, p in PROVIDERS.items():
        ...
        out.append((name, need is None, f"{status}; {p.note}" if p.note else status))
    return out
