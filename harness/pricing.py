...
CACHE = Path(__file__).resolve().parent.parent / ".cache" / "litellm_prices.json"
TTL = 7 * 24 * 3600
...
def cost_of(model: str, fresh_in: int, cached_in: int, out: int) -> float | None:
    ...
    return (
        fresh_in * inp
        + cached_in * (e.get("cache_read_input_token_cost") or inp)
        + out * (e.get("output_cost_per_token") or 0.0)
    )


def is_free(model: str) -> bool:
    """Локальные модели считаем бесплатными, а не 'цена неизвестна'."""
    return model.startswith(("ollama", "local")) or os.getenv("HARNESS_FREE_MODELS", "") == "1"
