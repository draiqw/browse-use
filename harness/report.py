"""Отчёты: строка на прогон и сводная таблица по парам (модель, профиль)."""
...
def line(rep: RunReport) -> str:
    ...
    s = (f"[{mark}] {rep.task} | {rep.model} | {rep.profile} | "
...
def table(reports: list[RunReport]) -> str:
    """Сводка по парам (модель, профиль): доля сошедшихся прогонов, медиана шагов, средняя цена."""
    groups: dict[tuple[str, str, str], list[RunReport]] = defaultdict(list)
    for r in reports:
        groups[(r.task, r.model, r.profile)].append(r)

    rows = []
    for (task, model, prof), rs in sorted(groups.items()):
        ...
        rows.append((task, model, prof, f"{good}/{len(rs)}",
                     f"{min(steps)}/{med}/{max(steps)}" if steps else "—",
                     f"{secs:.0f}s", _money(avg)))

    head = ("задача", "модель", "профиль", "сошлось", "шаги min/med/max", "время", "цена/прогон")
