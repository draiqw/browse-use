"""Локальная задача на клики: проверяет ровно тот провал, который мы нашли эмпирически.

Тогглы на странице — кастомные: сам input скрыт через opacity:0, видна только подпись.
Такие элементы не попадают в карту элементов browser-use, и агент, который умеет
кликать только по индексам, их не видит. Ровно так устроены фильтры в банковских
личных кабинетах.

Задача офлайновая и бесплатная (file://), эталон известен точно — это делает её
пригодной для регрессии: гоняй сколько хочешь, ответ всегда один.
"""
...
from harness.fixtures import PORT, url_for
from harness.task import Task, register

FIXTURE = Path(__file__).resolve().parent.parent / "fixtures" / "clickgate.html"
